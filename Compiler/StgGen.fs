...
let rec genExpr (topEnv: Map<Stg.Var, TopEnv>) e =
    let lf =
        ...
        | Core.Const constr -> genConst topEnv constr
        ...
        | Core.Unreachable -> genPrim topEnv Wasm.Unreachable [ Core.Const <| Core.Lit(Core.I32 -1) ]
    Stg.normLf lf

and genConst topEnv =
    function
    | Core.Constr(Core.UserConstr i) ->
        let (v, _) =
            topEnv
            |> Map.toSeq
            |> Seq.find (function
                | (var, IsConstr j) -> i = j
                | _ -> false)
        Stg.lambdaForm (Stg.Constr(v, []))
    | Core.Lit lit -> Stg.lambdaForm (Stg.Prim [ Stg.ALit(genLit topEnv lit) ])
    | Core.Prim w -> Stg.lambdaForm (Stg.Prim [ Stg.ALit w ])
...
and genLet topEnv e =
    function
    | Core.NonRec, [ v1, e1 ] ->
        genBindings topEnv genNonRec [ v1 |> snd, genExpr topEnv e1 ] (genExpr topEnv e)
...
and genBindings topEnv mapExpr (lfEs: (_ * Stg.LambdaForm) list) lf =
    let newStdConstrs, newLets =
        lfEs
        |> List.partition (fun (_, lfE) ->
            match lfE.args, lfE.expr with
            | [], Stg.Constr(_) -> true
            | _ -> false)
...
and genLetJoin topEnv (j, eJ) e =
    ...
    let expr = Stg.Let(Stg.Join(snd j, args, lfJ.expr), lf.expr)
...
and genAlts topEnv def =
    function
    | (Core.Constr v, vs, e) :: xs ->
        let lfDef = genExpr topEnv def

        let newTopEnv =
            if v = Core.IntConstr then withPrimBinders topEnv vs else topEnv
...
and genAAlts topEnv lfAcc def aalts =
    function
    | (Core.Constr v, vs, e) :: xs -> genAAlt topEnv lfAcc def aalts v (vs |> List.map snd) e xs
    | [] -> Stg.AAlts(aalts, def), lfAcc

and genConstr topEnv =
    function
    | Core.IntConstr -> 1
    | Core.BoolConstr -> 2
    | Core.UserConstr i -> 3 + i
...
and genPAlts topEnv lfAcc def (palts: Stg.PAlts) =
    function
    | (Core.Lit l, [], e) :: xs -> genPAlt topEnv lfAcc def palts l e xs
...
and genApp topEnv args =
    ...
    | Core.Const(Core.Prim p) -> genPrim topEnv (p) args

and genAppWithArgs topEnv (f: Stg.Var) args =
    let arity = forcedCallArity topEnv (f)
    match arity with
    | None -> genAtoms topEnv (genAppWithAtoms topEnv (f)) [] args
...
and genAtoms topEnv mapAtoms xs =
    ...
    | (Core.VarE arg) :: args when forcedCallArity topEnv (snd arg) = None ->
        ...
        { lf with frees = frees }
    | (Core.Const(Core.Lit arg)) :: args -> genAtoms topEnv mapAtoms (Stg.ALit(genLit topEnv arg) :: xs) args
    | arg :: args ->
...
and genPrim topEnv w ps = genAtoms topEnv (fun atoms -> Stg.lambdaForm (Stg.Prim(Stg.ALit w :: atoms))) [] ps

let rec genTopLam topEnv x args =
    function
...
let genTopLevelExpr topEnv = function
    | b, (_, e) ->
...
let genTopLevelConstr topEnv = function
    | b, (c, vs) -> snd b, Stg.TopConstr(genConstr topEnv c, vs |> List.map snd)

let rec getManifestArity =
    function
...
let rec uniqueifyExpr: Core.Expr<'a> -> Core.Expr<Stg.Var> =
    function
...
and uniqueifyAlts alts = alts |> List.map (fun (a, bs, e) -> a, bs |> List.map wrapBinder, uniqueifyExpr e)
