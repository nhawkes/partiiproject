...
let var : Ast.Var -> Core.Var<_> = fun v -> Core.F(Core.s2n v.text, {v=Some v; typ=Types.ValueT; hintInline=false})
let b : Ast.Var -> Core.Binder<_> = fun v -> (Core.s2n v.text, {v=Some v; typ=Types.ValueT; hintInline=false})
let genName =
    let i = ref (1)
    fun () ->
        let next = !i
...
let genProgram (ast: Ast.Program): Core.ClosedProgram<_> = 
    ...
    and genCases (def, (matchexpr, typ), es:(MatchExpr*Types.Typ) list, bind) subCases otherCases =
        ...
        | (Ast.PatBind(bindV) :: xs, e) :: cases ->
            genCases (def, (matchexpr, typ), es, Some (Core.s2n bindV.text, {v=Some bindV; typ=Types.ValueT; hintInline=false})) ((xs, e) :: subCases) otherCases cases
        | case :: cases -> genCases (def, (matchexpr, typ), es, bind) (subCases) (case :: otherCases) cases
        | [] ->
            let bindV = bind |> Option.defaultWith (fun () -> genName(), {v=None;typ=Types.ValueT; hintInline=false})
            let defVar = genName(), {v=None; typ=Types.ValueT; hintInline=false}
    ...
    and genCasesWithDefault (def, alts, (matchexpr, typ), es, bind) cases =
        ...
        | ((Ast.PatLit(Ast.Box(Ast.Integer _)) :: _, _)) :: xs ->
            genPatConstr state Core.IntDestr [ (genName(), {v=None; typ=Types.IntT; hintInline=false}), Types.IntT ] [] [] cases
        | ([ Ast.PatConstr(v, ps) ], e) :: xs ->
            let vs =
                ps
                |> List.map (function
                    | Ast.PatBind(v) ->b v
                    | _ -> genName(), {v=None; typ=Types.ValueT; hintInline=false})
    ...
    let rec genExport call args rhs =
        ...
        | [] ->
            let resultVar = genName(), {v=None; typ=Types.ValueT; hintInline=false}
            let returnVar = genName(), {v=None; typ=Types.IntT; hintInline=false}
            Core.caseE
                (call, resultVar,
    ...
    let rec genDeclarations fresh topConstrs topExprs  =
        ...
        | Ast.TypeDecl(Ast.AssignFunc(v, vs))::xs -> 
            genDeclarations fresh ((b v, (getConstr v, vs |> List.map(function Ast.AssignVar v -> v.text, {v=Some v; typ=Types.ValueT; hintInline=false})))::topConstrs) topExprs xs
        |[] ->
            ...
            Core.closeProgram {constrs=constrs; exprs=exprs;}
